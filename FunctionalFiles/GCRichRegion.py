...
Purpose : 
		...
		Output File : A html file indecating the GC rich region.
"""
...
class GCRichRegion(QtGui.QMainWindow):

  def __init__(self, parent=None):
    super(GCRichRegion, self).__init__()
  ...
  def GetRichRegion(self,subset_array,max_AT_array_counter,sequence,threshold):
	  ...
	  while prev_counter>=0 or next_counter<int(len(sequence)):
		  ...
		  if prev_counter>0 and flag_prev==1 :
			  ...
			  neumator=int(GC)+int(subset_array[max_AT_array_counter,4])
	  ...
	  if found ==0:
		  temp_array[0,0] = str(subset_array[max_AT_array_counter,0])
		  temp_array[0,1] = len(subset_array[max_AT_array_counter,0])
		  temp_array[0,2] = str(subset_array[max_AT_array_counter,2])
		  temp_array[0,3] = str(subset_array[max_AT_array_counter,3])
		  temp_array[0,4] = "100.00"		  
	  return temp_array
	   
  def GCRichRegion(self,sourcepath,output_file,percentage):
	  sequence_start =0
	  new_character = ""
	  prev_character = ""
	  temp = ""
	  ...
	  result_sequence_GC_percentage=0.00
...
# sub sequence.					  
					 subset_array[counter,0]=temp;
# length of sub sequence.					 
					 subset_array[counter,1]=int(len(temp))
# to set first postion of this subsequence formula is current position - length of sub sequence.					  
					 subset_array[counter,2]=i-int(len(temp)) 
...
# increment counter by one to set new subsequence					 
					 counter = counter +1  
...
# sub sequence.					  
					 subset_array[counter,0]=temp;
# length of sub sequence.					 
					 subset_array[counter,1]=len(temp); 
# to set first postion of this subsequence formula is current position - length of sub sequence.					 
					 subset_array[counter,2]=i-int(len(temp)); 
					 subset_array[counter,3]=i;
