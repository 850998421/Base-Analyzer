...
Version : 1.0
Purpose : Draw the Codon Usage Table With Comparision Of Base Sequence window 
			and import CodonUsaseWithComperision file to perform its functional part
"""
...
from FunctionalFiles.CodonUsaseWithComperision import codonUsaseTableWithComperision


class codonUsaseWithComperisionUI(QtGui.QDialog):
...
#create a window		
		...
		self.setWindowTitle('Codon Usage Table (With Comparision)')
		self.setModal(True)
...
#function to perform writing the output file			
	def saveFile(self):
		...
		if str(self.sourcefile)!= ""  and str(self.updated_file_name)!= "":
			self.W = codonUsaseTableWithComperision()
			codonusase = self.W.codonUsaseTableWithComperision(self.sourcefile,self.updated_file_name);
			hello = self.W.dataFileToDrawChart(self.updated_file_name);
